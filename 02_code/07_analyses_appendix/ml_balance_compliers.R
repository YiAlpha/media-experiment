...
ml_comp <- subset(ml, compliance == 1)
...
conditional_sims <-
	as.data.frame(do.call("rbind", conditional_mat))
...
names(baltab_conditional_comp) <- sort(unique(ml_comp$treatment))

conditional_baltab_comp <- cbind.data.frame(covariate = covariates,
																						round(
																							data.frame(baltab_conditional_comp[, c(
																								"placebo",
																								"IPV",
																								"abortion",
																								"absenteeism",
...
names(conditional_baltab_comp) <- c("covariate",
																		"PLA",
																		"IPV",
																		"ABO",
																		"ABS",
																		...
																		"IPV_ABO",
																		"p-value")
rownames(conditional_baltab_comp) <-
	conditional_baltab_comp$covariate
conditional_baltab_comp$covariate <- NULL

sink("03_tables/balance_conditional_comp_ml.tex")
print(
	xtable(conditional_baltab_comp),
