...
el_comp <- subset(el, respondent_category == "Complier")
...
for (j in (length(marginal_mat) + 1):sims) {
	new_data <- rerandomize(analysis_data = el_comp)
	null_lik_dif <- sapply(
		X = 1:length(covariates_el),
		FUN = function(i) {
			full_model_marginal <- lm(formula = as.formula(
...
conditional_baltab_comp <- cbind.data.frame(covariate = covariates_el,
																						round(
...
sink("03_tables/balance_conditional_compliers_el.tex")
print(
